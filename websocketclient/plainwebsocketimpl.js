...
export class PlainWebsocketImpl {

    constructor(endpoint, access_token) {
        console.log(`PlainWebsocketImpl constructor endpoint=${endpoint}, access_token=${access_token}`)
        this.appendLog(`{{Plain Websocket Implementation...., host=${endpoint}}}`)
        this.endpoint = endpoint
        this.access_token = access_token
    }

    connect() {
        this.socket = new WebSocket(`${this.endpoint}?access_token=${this.access_token}`);
    ...
    onMessageReceivedCallback(event) {
        ...
        this.appendLog(`Received message: ${message}`)
        ...
        if (command == 'CONNECTED') {
            ...
            this.subscribe('/user/queue/userNotifications', 'vk-0');
        }
